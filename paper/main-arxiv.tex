...
\begin{enumerate}
    ...
    \item \textbf{Implementation Properties}: Array bounds (MOUNTH macro), integer overflow (fd allocation), reference count safety (CBMC: 113 properties, 0 failures)
\end{enumerate}
...
counting (45 properties): 113 total properties, 0 failures. Full-mode harnesses
verify the complete \texttt{pgrpcpy()} implementation with \texttt{MNTHASH=32}
(production configuration), \texttt{--unwind 34}, and \texttt{--object-bits 11}.
...
implements the models. CBMC partially closes this gap by verifying properties on actual
C source. The multi-tool approach---abstract specification, concurrent protocol
verification, and implementation-level bounded model checking---provides complementary
coverage: each tool catches different classes of bugs.
...
\begin{enumerate}
    ...
    \textit{(Verified by TLA+, SPIN, and CBMC on C source.)}
