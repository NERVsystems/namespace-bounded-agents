...
\begin{abstract}
Large language model agents reach external services through tool-calling protocols, creating an attack surface for prompt injection and capability abuse. Current defenses---input sanitization, output filtering, heuristic guardrails---can be bypassed adversarially and ultimately depend on model alignment.

We propose namespace-bounded agents: agents run inside Plan 9-style namespaces in which capabilities are filesystem paths rather than API schemas. An agent cannot invoke, or be manipulated into invoking, a capability outside its mounted namespace: the path does not exist in its execution context. This is capability-based security enforced by the operating system, with structural guarantees independent of model behavior.

We implement the architecture on Inferno OS over the 9P protocol and evaluate it three ways. On a 31-attack corpus spanning nine vulnerability categories, three models (Claude Sonnet 4, GPT-5, GPT-4o; 372 trials) yield a 0\% attack success rate on in-scope attacks: models that refuse are blocked behaviorally, and models that attempt attacks are blocked structurally by the kernel. On the AgentDojo benchmark (629 injection attacks), evaluated through a tool-visibility filter replicating namespace semantics, 75.2\% of attacks require cross-tool access and fail by construction, while same-tool attacks succeed 2.6\%, for 0.6\% overall. Filesystem semantics also cut context token consumption by 83.5\% relative to JSON schemas, turn tool-schema poisoning from a semantic attack into a syntactic, implementation-verifiable one, and need no explicit schemas. We formally verify the isolation mechanism with TLA+/TLC, SPIN, and CBMC on kernel C source, with zero violations, and uncover three previously undocumented use-after-free races. Prototype and verification artifacts are open-source.
\end{abstract}
