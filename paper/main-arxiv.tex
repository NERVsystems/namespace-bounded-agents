...
We implement this architecture using Inferno OS and the 9P protocol, demonstrating it with geospatial services. Our evaluation across a 31-attack corpus spanning nine vulnerability categories shows that: (1) multi-model validation (Claude Sonnet 4, GPT-5, GPT-4o) with 4-run statistical validation (n=372) achieves \textbf{0\% attack success rate} (95\% CI: [0.000, 0.008]) through defense-in-depth---behavioral blocking when models refuse, structural blocking at the OS level when they attempt attacks; (2) filesystem semantics reduce token usage by 83.5\% compared to JSON schema approaches; (3) tool schema poisoning attacks that are semantic in MCP become syntactic in 9P, shifting from model-dependent to implementation-verifiable defenses; and (4) LLMs correctly infer filesystem-exposed tool usage without explicit schemas. Evaluation against the AgentDojo benchmark (629 injection attacks, 4 domains), using a tool-visibility filter that replicates namespace semantics, demonstrates that 75.2\% of attacks require cross-tool access and achieve \textbf{0\% attack success rate} under namespace isolation (guaranteed by construction; in 32.8\% of these the agent attempted the out-of-namespace access and was blocked), while the remaining 24.8\% same-tool attacks achieve 2.6\% success rate (behavioral defense), yielding \textbf{0.6\% overall ASR}. We formally verify namespace isolation using three complementary tools: TLA+/TLC model checking (369 million states exhaustive, 3.17 billion states partial; 11 safety invariants, 0 violations), SPIN (131,333 states across 5 concurrent protocol models, all properties verified), and CBMC bounded model checking of C implementation code (113 properties, 0 failures). One SPIN model, built to detect races, additionally reproduces three previously undocumented use-after-free race conditions in the kernel's host threading layer. Our prototype and verification artifacts are open-source.
\end{abstract}
...
\begin{enumerate}
    ...
    \item We validate against the AgentDojo benchmark (629 injection attacks, 4 domains) via a tool-visibility filter replicating namespace semantics: 75.2\% of attacks are cross-tool and achieve 0\% ASR (guaranteed by construction; 32.8\% of them attempted and blocked), 24.8\% same-tool attacks achieve 2.6\% ASR (behavioral), yielding 0.6\% overall ASR compared to 53.1\% for behavioral defenses alone and 7.5\% for AgentDojo's tool-filter defense
    \item We formally verify namespace isolation properties using TLA+/TLC (369M states exhaustive, 3.17B partial; 11 invariants, 0 violations), SPIN (131K states, 5 models, 0 errors), and CBMC (113 properties, 0 failures), and discover three real use-after-free race conditions in the kernel
...
\hline
Capability isolation & Namespace boundary & TLA+ (369M states) & Same-tool misuse \\
Path traversal block & Walk() validation & TLA+, SPIN & Covert channels \\
Reference count safety & Kernel ref counting & TLA+, CBMC (113) & Timing attacks \\
Post-fork isolation & pgrpcpy() semantics & TLA+, SPIN, CBMC & Social engineering \\
Lock ordering & pg->ns before mh->lock & SPIN (39,744 states) & Default mounts \\
Export root boundary & Walk("..") validation & SPIN (106 states) & Network attacks \\
\hline
\end{tabular}
\caption{Verified properties and explicit non-coverage (summary). See Section~\ref{sec:evaluation} for detailed verification methodology, assumptions, and scope. Structural claims rest on Guarantee 1 (Section~\ref{sec:design}); empirical attack rates (Tables~\ref{tab:multi-model},~\ref{tab:agentdojo-results}) provide validation but are not the basis of the guarantee.}
\label{tab:tcb}
\end{table}
...
\noindent We state this as a \textit{guarantee} with explicit assumptions rather than as a theorem: the argument below is informal, and the machine-checked formal objects underlying it are the TLA+ Namespace Isolation Theorem and path-confinement lemmas of Section~\ref{sec:evaluation}.
...
\textbf{seL4} \cite{klein2009sel4}: The first formally verified general-purpose OS kernel. seL4 uses capabilities for all system resource access, with machine-checked proofs of isolation. Our formal verification draws on seL4's methodology using model checking (TLA+, SPIN) rather than theorem proving (Isabelle/HOL), trading completeness for automation, with CBMC providing implementation-level verification on actual C source.
...
\begin{itemize}
    ...
    \item \textbf{Formal guarantees}: Multi-tool verification---TLA+/TLC (369M states exhaustive, 3.17B partial, 11 invariants, 0 violations), SPIN (131K states, 5 models), and CBMC (113 properties on C source)---confirms namespace isolation properties and discovers 3 real race conditions
...
As LLM agents become more capable and autonomous, the security of their capability interfaces becomes critical. Namespace isolation provides a formally grounded primitive that structurally prevents cross-tool attacks (under Guarantee 1 assumptions) rather than relying on behavioral compliance---applying established OS capability primitives to modern AI safety challenges.
